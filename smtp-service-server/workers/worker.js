...
//     const claim = await Email.findOneAndUpdate(
...
//     emitEmailEvent(claim._id.toString(), "sending");

//     try {
//       const valid = await validateEmailDomain(claim.to);
//       if (!valid) {
...
//           }
//         );
//         emitEmailEvent(claim._id.toString(), { status: "failed", error: msg });
//         return;
//       }

//       const isPublic = PUBLIC_TYPES.includes(claim.type);
//       let config, sender, cacheKey;
...
//       } else {
//         const apiKey = await ApiKey.findOne({ user: claim.user });
...
//       emitEmailEvent(claim._id.toString(), "sent");
//       info(`sent ${claim._id}`);
//     } catch (e) {
//       const msg = e?.message || "Unknown email send error";
//       const delay = Math.min(30000 * Math.pow(2, email.attempts || 0), 300000);
//       await Email.updateOne(
//         { _id: email._id },
//         {
//           $set: {
...
//         }
//       );
//       emitEmailEvent(email._id.toString(), { status: "failed", error: msg });
//       error(`fail ${email._id}: ${msg}`);
//     }
//   }
...
//       await Promise.allSettled(c.map((e) => processSingle(e)));
...
function createWorker(env) {
  ...
  async function processClaimed(claimed) {
    ...
    try {
      ...
      const isPublic = PUBLIC_TYPES.includes(claimed.type);
      ...
      if (isPublic) {
        ...
        sender = cred.appUserEmail;
        cacheKey = `user_${cred.appUserEmail}`;
        info(`📧 Worker ${WORKER_ID}: USER SMTP for ${claimed.to}`);
      }
